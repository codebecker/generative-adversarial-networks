...
class Generator(nn.Module):
    ...
    def _init_modules(self):
        ...
        # Project the input
        self.linear1 = nn.Linear(self.nz, 512*4*4, bias=False)
        self.bn1d1 = nn.BatchNorm1d(512*4*4) if self.batchnorm else None
        ...
        self.conv0 = nn.Conv2d(
                in_channels=512,
                out_channels=256,
                kernel_size=1,      #ouput dim :[(4−1+2*0)/1]+1 =4
                stride=1,
                padding=0,
                bias=False)
        self.bn2d0 = nn.BatchNorm2d(256) if self.batchnorm else None
        
        self.conv1 = nn.ConvTranspose2d(
                in_channels=256,
                out_channels=128,
                kernel_size=4,      #ouput dim :(4-1)*1-2*0+4=7
                stride=1,
                padding=0,
                bias=False)
...
# or Transpose Convolutions output dim:(W−1)*S-2P+(K-1)+1 ,W = width of Input, K= Kernel Size(Square) P=Padding, S= Stride
    ...
    def forward(self, input_tensor, embedding=None):
        ...
        intermediate = intermediate.view((-1, 512, 4, 4))
        
        intermediate = self.conv0(intermediate)
        if self.batchnorm:
            intermediate = self.bn2d0(intermediate)
        intermediate = self.leaky_relu(intermediate)
...
class Discriminator(nn.Module):
    ...
    def _init_modules(self):
        ...
        self.conv3 = nn.Conv2d(
                ...
                out_channels=256,        
                kernel_size=4,          
                stride=1,               #ouput dim Cifar:[(7−4+2*0)/1]+1 =4
                padding=0,
                bias=True)

        self.linear1 = nn.Linear(256*4*4, 1, bias=True)
